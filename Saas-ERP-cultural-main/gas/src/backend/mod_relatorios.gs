...
function excluirContrato(id, email) {
  ...
  try {
    ...
    Logger.error('relatorios', 'excluirContrato', e.message);
    return false;
  } finally {
    lock.releaseLock();
  }
...
function atualizarContrato(id, campos, email) {
  try {
    ...
    Logger.error('relatorios', 'atualizarContrato', e.message);
    return false;
  }
}
...
function excluirMeta(id, email) {
  ...
  try {
    ...
    Logger.error('relatorios', 'excluirMeta', e.message);
    return false;
  } finally {
    lock.releaseLock();
  }
...
function atualizarMeta(id, campos, email) {
  try {
    ...
    Logger.error('relatorios', 'atualizarMeta', e.message);
    return false;
  }
}
...
function excluirIndicador(id, email) {
  ...
  try {
    ...
    Logger.error('relatorios', 'excluirIndicador', e.message);
    return false;
  } finally {
    lock.releaseLock();
  }
...
function atualizarIndicador(id, campos, email) {
  try {
    ...
    Logger.error('relatorios', 'atualizarIndicador', e.message);
    return false;
  }
}
...
function salvarRubrica(dados, email) {
  ...
  try {
    ...
    Logger.info('relatorios', 'salvarRubrica', { id: dados.id || null, idMeta: dados.idMeta || null, nome: dados.nome || null, itens: (dados.memoriaCalculo || []).length });
    ...
    try {
      ...
      Logger.warn('relatorios', 'Histórico não salvo', e.message);
    }
    ...
    try {
      ...
      Logger.warn('relatorios', 'Versionamento', e.message);
    }

    return true;

  } catch (e) {
    Logger.error('relatorios', 'salvarRubrica', e.message);
    throw e;
...
function obterMemoriaRubrica(idRubrica) {
  ...
  if (!aba || aba.getLastRow() < 2) {
    Logger.warn('relatorios', 'obterMemoriaRubrica: aba vazia ou não encontrada');
    return [];
  }
...
function excluirRubrica(id, email) {
  ...
  try {
    ...
    Logger.error('relatorios', 'excluirRubrica', e.message);
    return false;
  } finally {
    lock.releaseLock();
  }
...
function atualizarRubrica(id, campos, email) {
  try {
    ...
    Logger.error('relatorios', 'atualizarRubrica', e.message);
    return false;
  }
}
