...
function obterDadosParaConfig(nomeAba) {
  try {
    ...
    Logger.error('admin', 'Erro ao buscar dados', e.message);
    return [];
  }
}
...
Este e-mail foi gerado automaticamente pelo Sistema de Gestão de Espaços do CCBJ.
    ...
    Logger.error('admin', 'Erro ao notificar', e.message);
  }
}
...
function chat_criarSolicitacao(tipo, subtipo, dados, usuario, justificativa) {
  ...
  try {
    ...
    try {
      ...
      Logger.warn('admin', 'Notificação falhou (não crítico)', e.message);
    }
...
function aprovarSolicitacao(id, emailAprovador) {
  ...
  try {
    ...
    SystemEvents.emit(SystemEventTypes.RESERVATION_APPROVED, {
      ...
      usuario: emailAprovador, origem: 'mod_admin'
    });
