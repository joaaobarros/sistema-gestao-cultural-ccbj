...
function _sincronizarEdicaoComRece(dados) {
  try {
    ...
    const linhasRece = abaRece
      .getRange(2, 1, abaRece.getLastRow() - 1, 24)
      .getValues();
    ...
    const tituloNovo = String(dados.nomeAcao || "")
      .trim()
      .toUpperCase();
    ...
    for (let i = 0; i < linhasRece.length; i++) {
      const idVinculo = String(linhasRece[i][23] || "").trim();
      const tituloRece = String(linhasRece[i][1] || "")
        .trim()
        .toUpperCase();
      const dataRece = normData(linhasRece[i][2]);

      const vinculado =
        (idReserva && idVinculo === idReserva) ||
      ...
      abaRece.getRange(linhaRece, 2).setValue(dados.nomeAcao);
      abaRece.getRange(linhaRece, 3).setValue(dataFormatada);
      abaRece.getRange(linhaRece, 5).setValue(dados.horaInicio);
      abaRece.getRange(linhaRece, 6).setValue(dados.horaTermino);
      abaRece.getRange(linhaRece, 7).setValue(nomeEspaco);
    ...
    Logger.error('comunicacao', 'sincronizarRECE', e.message);
  }
}
...
function verificarPermissaoRece(emailUsuario) {
  try {
    ...
    for (let i = 1; i < dados.length; i++) {
      const ep = String(dados[i][0] || "")
        .toLowerCase()
        .trim();
      const nivel = String(dados[i][1] || "")
        .toLowerCase()
        .trim();
      if (ep === email) {
        return ["admin", "superadmin", "comunicação", "comunicacao"].includes(
          nivel,
        );
      }
    }
