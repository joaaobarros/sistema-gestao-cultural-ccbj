...
// ══════════════════════════════════════════════════════════════════
// BLOCO: Helpers de acesso a Configuracoes (aba sem repositório)
// ══════════════════════════════════════════════════════════════════
...
function _chvVerificarChaveJaEmUso(chaveId) {
  const protocolos = ChavesRepository.listarProtocolos();
  const statusAtivos = [
...
function chaves_obterDados(emailAtual) {
  try {
    ...
    const chaves = ChavesRepository.listarChaves().filter(function(c) { return c && c.ativa; });

    const todos = ChavesRepository.listarProtocolos();
...
function chaves_listarProtocolos(filtros, emailAtual) {
  try {
    ...
    let lista = ChavesRepository.listarProtocolos();
  ...
  try {
    ...
    try {
      ...
      const novoId = ChavesRepository.criarProtocolo({
        ...
        espacoId:            dados.espacoId,
        responsavelId:       email,
        responsavelNome:     nomeInfra,
        solicitanteId:       dados.destinoEmail,
        solicitanteNome:     nomeDestino,
        setorId:             setor,
        setorNome:           setor,
        dtPrevistaDevolucao: String(dados.dtPrevistaDevolucao || ''),
        status:              CHV_STATUS_PROTOCOLO.AGUARDANDO_CONFIRMACAO_USUARIO,
        observacoes:         String(dados.observacoes || ''),
        entreguePorId:       email,
        entreguePorNome:     nomeInfra,
        reservaVinculadaId:  String(dados.reservaId || ''),
        origem:              'ENTREGA_DIRETA'
      });

      ChavesRepository.atualizarStatusChave(dados.chaveId, CHV_STATUS_CHAVE.EM_USO);
      ChavesRepository.appendHistorico({
        protocoloId: novoId, chaveId: dados.chaveId,
        acao: 'ENTREGA_DIRETA_INICIADA',
        usuarioId: email, usuarioNome: nomeInfra,
        statusAnterior: CHV_STATUS_CHAVE.DISPONIVEL,
        statusNovo: CHV_STATUS_PROTOCOLO.AGUARDANDO_CONFIRMACAO_USUARIO,
        observacoes: dados.observacoes || '', agente: 'INFRA'
      });
...
function chaves_aprovarEntrega(protocoloId, obs, emailAtual) {
  try {
    ...
    try {
      const p = ChavesRepository.obterProtocoloPorId(protocoloId);
      if (!p) throw new Error('Protocolo não encontrado: ' + protocoloId);
      ...
      ChavesRepository.atualizarStatusChave(p.chaveId, CHV_STATUS_CHAVE.EM_USO);
...
function chaves_confirmarDevolucao(protocoloId, obs, emailAtual) {
  try {
    ...
    try {
      ...
      ChavesRepository.atualizarStatusChave(p.chaveId, CHV_STATUS_CHAVE.DISPONIVEL);
...
function chaves_solicitarTransferencia(protocoloId, destinoEmail, obs, emailAtual) {
  try {
    ...
    try {
      ...
      const chave = ChavesRepository.obterChavePorId(p.chaveId);
      if (chave) {
        if ([CHV_STATUS_CHAVE.BLOQUEADA, CHV_STATUS_CHAVE.EXTRAVIADA].includes(chave.status))
          throw new Error('Não é possível transferir chave com status: ' + chave.status);
      }
...
function chaves_obterIndicadores(emailAtual) {
  try {
    obterEmailUsuario(emailAtual || '');
    const protocolos = ChavesRepository.listarProtocolos();
    const chaves     = ChavesRepository.listarChaves();
  ...
  try {
    ...
    try {
      ...
      const novoId = ChavesRepository.criarProtocolo({
        ...
        solicitanteId:       '',
        solicitanteNome:     destinoNome,
        setorId:             setor,
        setorNome:           setor,
        dtRetirada:          agora,
        dtPrevistaDevolucao: String(dados.dtPrevistaDevolucao || ''),
        status:              CHV_STATUS_PROTOCOLO.RETIRADA,
        observacoes:         String(dados.observacoes || ''),
        entreguePorId:       email,
        entreguePorNome:     nomeInfra,
        recebidoPorId:       '',
        recebidoPorNome:     destinoNome,
        reservaVinculadaId:  String(dados.reservaId || ''),
        origem:              'RETIRADA_OPERACIONAL'
      });

      ChavesRepository.atualizarStatusChave(dados.chaveId, CHV_STATUS_CHAVE.EM_USO);
      ChavesRepository.appendHistorico({
        protocoloId: novoId, chaveId: dados.chaveId,
        acao: 'RETIRADA_OPERACIONAL',
        usuarioId: email, usuarioNome: nomeInfra,
        statusAnterior: CHV_STATUS_CHAVE.DISPONIVEL,
        statusNovo: CHV_STATUS_PROTOCOLO.RETIRADA,
        observacoes: 'Para: ' + destinoNome + (setor ? ' (' + setor + ')' : '') + '. ' + (dados.observacoes || ''),
        agente: 'INFRA'
      });
